...
const updateMe = (id, input) => withTransaction(async (client) => {
  ...
  const fullName = input.fullName === undefined ? null : String(input.fullName).trim();
  const email = input.email === undefined ? null : String(input.email).trim().toLowerCase();
  const phone = input.phone === undefined ? null : String(input.phone).trim();
  const address = input.address === undefined ? null : String(input.address).trim();
  ...
  if (customer) {
    if (email) {
      ...
      if (duplicate.rows[0]) throw new ErrorHandler("Email da duoc su dung", 409);
    }
    const encryptedEmail = input.email === undefined ? null : encryptProfileValue(email);
    const encryptedPhone = input.phone === undefined ? null : encryptProfileValue(phone);
    const encryptedAddress = input.address === undefined ? null : encryptProfileValue(address);
    await client.query(
      ...
      [id, fullName, encryptedEmail, email ? emailLookupHash(email) : null,
        input.phone !== undefined, encryptedPhone],
    );
    await client.query(
      ...
      [id, fullName, encryptedEmail, input.phone !== undefined, encryptedPhone,
        input.address !== undefined, encryptedAddress],
    );
  } else {
    await client.query(
...
const updateAvatar = async (user, file) => {
  ...
  try {
    ...
    const previous = await withTransaction(async (client) => {
      ...
      if (!current.rows[0]) throw new ErrorHandler('Không tìm thấy người dùng', 404);
      await client.query(
    ...
    try {
      if (previous.avatar_public_id) await uploader.destroyImage(previous.avatar_public_id);
    } catch (cleanupError) {
      await database.query(
        `UPDATE users SET avatar_url=$2,avatar_public_id=$3,avatar_size_bytes=$4,updated_at=NOW()
         WHERE id=$1 AND avatar_public_id=$5`,
        [user.id, previous.avatar_url, previous.avatar_public_id, previous.avatar_size_bytes, uploaded.public_id],
      );
    ...
    if (reservation && previous.avatar_size_bytes)
      await storageQuota.release(reservation.customerId, previous.avatar_size_bytes);
    return getById(user.id);
  } catch (error) {
    if (!uploaded) await storageQuota.releaseReservation(reservation);
    else {
      const saved = await database.query('SELECT 1 FROM users WHERE id=$1 AND avatar_public_id=$2', [user.id, uploaded.public_id]);
      if (!saved.rows[0]) await Promise.allSettled([
        uploader.destroyImage(uploaded.public_id),
        storageQuota.releaseReservation(reservation),
      ]);
    }
