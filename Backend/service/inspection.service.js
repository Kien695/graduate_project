...
const start = (id, user, ipAddress) => withTransaction(async (client) => {
  const old = await client.query("SELECT * FROM inspections WHERE id=$1 FOR UPDATE", [id]);
...
const addImages = async (id, files) => {
  ...
  await get(id);
  const uploaded = await Promise.all(files.map((file) => uploader.uploadBuffer(file)));
  const saved = [];
  for (const image of uploaded) {
    const { rows } = await database.query(
      "INSERT INTO inspection_images(inspection_id,url,public_id,image_url) VALUES($1,$2,$3,$2) RETURNING *",
...
module.exports = { list, get, create, update, start, updateChecklist, setStatus, addImages };
