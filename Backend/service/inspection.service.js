...
const lockInspection = (client, id) => client.query(
...
const create = (input, user) => withTransaction(async (client) => {
  ...
  await notifyInspection(client, { ...rows[0], customer_id: relation.rows[0].customer_id }, `Phiếu kiểm định cho đơn hàng #${input.order_id} đã được tạo.`);
  return rows[0];
});

const update = (id, input) => withTransaction(async (client) => {
  const old = await lockInspection(client, id);
  if (!old.rows[0]) throw new ErrorHandler("Không tìm thấy phiếu kiểm định", 404);
  const { rows } = await client.query(
    `UPDATE inspections
  ...
  if (!rows[0]) throw new ErrorHandler("Phiếu không tồn tại hoặc đã hoàn thành", 409);
  await notifyInspection(client, old.rows[0], `Thông tin kiểm định cho đơn hàng #${old.rows[0].order_id} đã được cập nhật.`);
  return rows[0];
});

const start = (id, user, ipAddress) => withTransaction(async (client) => {
  const old = await lockInspection(client, id);
  if (!old.rows[0]) throw new ErrorHandler("Không tìm thấy phiếu kiểm định", 404);
  if (String(old.rows[0].status).toLowerCase() !== "pending")
    throw new ErrorHandler("Chỉ có thể bắt đầu phiếu đang chờ kiểm định", 409);
  const { rows } = await client.query(
    "UPDATE inspections SET status='checking',updated_at=NOW() WHERE id=$1 RETURNING *",
    [id],
  );
  await audit.record(client, { userId: user.id, action: "CHECKING", entityType: "inspection", entityId: id, oldValues: old.rows[0], newValues: rows[0], ipAddress });
  await notifyInspection(client, old.rows[0], `Kiểm định cho đơn hàng #${old.rows[0].order_id} đã bắt đầu.`);
  return rows[0];
});

const updateChecklist = (id, checklist, notes, user, ipAddress) => withTransaction(async (client) => {
  const old = await lockInspection(client, id);
  if (!old.rows[0]) throw new ErrorHandler("Không tìm thấy phiếu kiểm định", 404);
  if (String(old.rows[0].status).toLowerCase() !== "checking")
    throw new ErrorHandler("Checklist chỉ được cập nhật khi đang kiểm định", 409);
  const { rows } = await client.query(
    `UPDATE inspections
  ...
  await audit.record(client, { userId: user.id, action: "CHECKLIST", entityType: "inspection", entityId: id, oldValues: { checklist: old.rows[0].checklist, notes: old.rows[0].notes }, newValues: { checklist: rows[0].checklist, notes: rows[0].notes }, ipAddress });
  await notifyInspection(client, old.rows[0], `Checklist kiểm định cho đơn hàng #${old.rows[0].order_id} đã được cập nhật.`);
  return rows[0];
});
...
const setStatus = (id, status, user, ipAddress) => withTransaction(async (client) => {
  ...
  if (!old.rows[0]) throw new ErrorHandler("Không tìm thấy phiếu kiểm định", 404);
  if (String(old.rows[0].status).toLowerCase() !== "checking")
    throw new ErrorHandler("Phiếu phải ở trạng thái đang kiểm định trước khi hoàn thành", 409);
  if (status === "passed" && (!old.rows[0].checklist?.length || old.rows[0].checklist.some((item) => !item.checked)))
    throw new ErrorHandler("Phải hoàn thành toàn bộ checklist trước khi đánh dấu PASS", 409);
  ...
  await audit.record(client, { userId: user.id, action: result, entityType: "inspection", entityId: id, oldValues: old.rows[0], newValues: rows[0], ipAddress });
  await notifyInspection(client, old.rows[0], `Kiểm định cho đơn hàng #${old.rows[0].order_id} có kết quả ${result}.`);
  return rows[0];
});
...
const addImages = async (id, files, user) => {
  ...
  let uploaded = [];
  try {
    uploaded = storageQuota.tagImages(await uploader.uploadMany(files), reservation);
    return await withTransaction(async (client) => {
      ...
      for (const image of uploaded) {
        const { rows } = await client.query(
          ...
          [id, image.url, image.public_id, image.bytes, image.customer_id || null],
        );
        saved.push(rows[0]);
      }
      return saved;
...
module.exports = { list, get, create, update, start, updateChecklist, setStatus, addImages, removeImage };
