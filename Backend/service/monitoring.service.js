...
const cpu = () => {
  ...
  for (const core of cpus) {
    ...
    total += Object.values(core.times).reduce((a, b) => a + b, 0);
  }
  return {
    cores: cpus.length,
    usagePercent: Number(((1 - idle / total) * 100).toFixed(2)),
  };
};
const load = () => ({ loadAverage: os.loadavg(), uptimeSeconds: os.uptime() });
const recordRequest = (durationMs, statusCode) => {
  requestState.count += 1;
  requestState.totalDurationMs += durationMs;
...
const requestMetrics = () => {
  ...
  return {
    ...
    errorCount: requestState.errors,
    errorRate: requestState.count
      ? requestState.errors / requestState.count
      : 0,
    averageLatencyMs: requestState.count
      ? requestState.totalDurationMs / requestState.count
      : 0,
...
const configure = async (input, userId) => {
  const existing = await database.query(
    "SELECT id FROM monitoring_alert_configs ORDER BY id LIMIT 1",
  );
  const values = [
    Number(input.cpuThreshold),
    Number(input.memoryThreshold),
    Number(input.loadThreshold),
    Number(input.responseTimeThresholdMs || 2000),
    userId,
  ];
  const { rows } = existing.rows[0]
    ...
    : await database.query(
       ...
       response_time_threshold_ms,updated_by) VALUES($1,$2,$3,$4,$5) RETURNING *`,
        values,
      );
  return rows[0];
};
...
const deliverAlert = async (metric, value, threshold) => {
  ...
  await database.query(
    `INSERT INTO monitoring_alerts(metric,measured_value,threshold_value,message,delivery_status)
     VALUES($1,$2,$3,$4,$5)`,
    [metric, value, threshold, message, deliveryStatus],
  );
};
const evaluateAlerts = async () => {
  const config = await database.query(
    "SELECT * FROM monitoring_alert_configs ORDER BY id LIMIT 1",
  );
  const thresholds = config.rows[0] || {
    cpu_threshold: 80,
    memory_threshold: 80,
  ...
  const snapshot = {
    ...
    latency: requestMetrics().p95LatencyMs,
  };
  if (snapshot.cpu > Number(thresholds.cpu_threshold))
    await deliverAlert("cpu_percent", snapshot.cpu, thresholds.cpu_threshold);
  if (snapshot.memory > Number(thresholds.memory_threshold))
    await deliverAlert(
      "memory_percent",
      snapshot.memory,
      thresholds.memory_threshold,
    );
  if (snapshot.latency > Number(thresholds.response_time_threshold_ms))
    await deliverAlert(
      "p95_latency_ms",
      snapshot.latency,
...
const startAlertWorker = () => {
  if (
    String(process.env.MONITORING_ENABLED || "true").toLowerCase() === "false"
  )
    return null;
  const timer = setInterval(
    () =>
      evaluateAlerts().catch((error) =>
        console.error("Monitoring worker failed:", error.message),
      ),
    Number(process.env.MONITORING_INTERVAL_MS || 60000),
  );
  timer.unref();
  return timer;
};
