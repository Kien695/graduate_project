...
const IDLE_TIMEOUT_MINUTES = Math.max(1, Number(process.env.SESSION_IDLE_TIMEOUT_MINUTES || 30));
...
const touch = async ({ sessionId, userId, refreshTokenHash }) => {
  ...
  const user = await withTransaction(async (client) => {
    ...
    if (refreshTokenHash && session.refresh_token_hash !== refreshTokenHash) return null;
    const lastActivity = new Date(session.last_activity_at || session.created_at);
    if (Date.now() - lastActivity.getTime() > IDLE_TIMEOUT_MINUTES * 60 * 1000) {
      await client.query(
    ...
    await client.query("UPDATE user_sessions SET last_activity_at=NOW() WHERE id=$1", [sessionId]);
    return session;
  });
  if (idleExpired) throw new ErrorHandler("Phien dang nhap da het han do khong hoat dong", 401);
  return user;
};
