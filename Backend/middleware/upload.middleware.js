...
const validateImageContent = (req, res, next) => {
  if (!req.file) return next(new ErrorHandler("Vui lòng chọn ảnh đại diện", 400));
  const actualMime = detectImageMime(req.file.buffer);
  if (!actualMime || actualMime !== req.file.mimetype)
    return next(new ErrorHandler("Nội dung file không đúng định dạng JPEG, PNG hoặc WEBP", 400));
