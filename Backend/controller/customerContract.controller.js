...
const list = catchAsyncError(async (req, res) =>
  successResponse(res, 200, 'Lấy danh sách hợp đồng thành công', await service.list(req.user.id)),
);

const get = catchAsyncError(async (req, res) =>
  successResponse(res, 200, 'Lấy hợp đồng thành công', await service.get(parseId(req.params.id), req.user.id)),
);
