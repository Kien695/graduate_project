...
export default function AdminLayout() {
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-700 dark:bg-slate-950 dark:text-slate-300">
    ...
    <div className="lg:pl-64"><header className="sticky top-0 z-20 flex h-20 items-center justify-between border-b border-slate-200 bg-white/95 px-4 backdrop-blur dark:border-slate-800 dark:bg-slate-900/95 sm:px-7"><div className="flex items-center gap-4"><button onClick={() => setOpen(true)} className="rounded-xl border border-slate-200 p-2 dark:border-slate-700 lg:hidden"><Icon name="menu" /></button><div className="hidden items-center gap-2 rounded-xl bg-slate-100 px-4 dark:bg-slate-800 sm:flex"><Icon name="search" className="h-4 w-4 text-slate-400" /><input className="w-64 bg-transparent py-2.5 text-sm outline-none dark:text-slate-100 dark:placeholder:text-slate-500" placeholder="Tìm kiếm nhanh..." /></div></div><div className="flex items-center gap-4"><ThemeToggle /><div className="h-9 w-px bg-slate-200 dark:bg-slate-700" /><UserMenu user={user} /></div></header><main className="p-4 sm:p-7"><Outlet /></main></div>
  </div>;
}
