...
export default function ProfilePage() {
  ...
  useEffect(() => {
    dispatch(loadCurrentUser())
      .unwrap()
      .then((freshUser) => setProfile(profileValues(freshUser)))
      .catch(() => undefined);
  }, [dispatch]);
  useEffect(
    () => () => {
  ...
  const uploadAvatar = async () => {
    ...
    try {
      ...
      toast.error(
        typeof error === "string" ? error : "Không thể cập nhật ảnh đại diện",
      );
    } finally {
  ...
  const saveProfile = async (event) => {
    ...
    try {
      ...
      toast.error(
        typeof error === "string" ? error : "Không thể cập nhật hồ sơ",
      );
    } finally {
      setSaving(false);
    }
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          {avatarFile && (
            <button
              ...
              className="rounded-lg bg-blue-600 px-3 py-1.5 text-[11px] font-bold text-white disabled:opacity-60"
            >
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        >
          ...
          <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
            Dữ liệu được tải trực tiếp từ tài khoản đang xác thực.
          </p>
          <div className="mt-5 space-y-4">
            ...
            <Field
              ...
              disabled
              placeholder="Chưa có dữ liệu địa chỉ cho tài khoản nhân viên"
            />
            <Field
              label="Vai trò"
              value={
          ...
          <button
            disabled={saving}
            className="mt-6 w-full rounded-xl bg-blue-600 py-3 text-sm font-bold text-white disabled:opacity-60"
          >
        ...
        <form
          ...
          className="h-fit rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-800"
        >
          <h2 className="text-lg font-black text-slate-900 dark:text-white">
            Đổi mật khẩu
          </h2>
          ...
          <button
            ...
            className="mt-6 w-full rounded-xl bg-slate-900 py-3 text-sm font-bold text-white disabled:opacity-60"
          >
...
function Field({ label, ...props }) {
  return (
    <label className="block">
      ...
      <input
        {...props}
        className="form-control disabled:bg-slate-100 disabled:text-slate-500 dark:disabled:bg-slate-900 dark:disabled:text-slate-500"
      />
    </label>
  );
}
