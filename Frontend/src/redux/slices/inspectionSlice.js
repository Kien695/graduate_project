...
export const fetchInspections = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      return (await getData("/inspections")).data;
    } catch (error) {
      return rejectMessage(error, "Không thể tải kiểm định", rejectWithValue);
    }
  },
);
export const createInspection = createAsyncThunk(
  ...
  async (data, { rejectWithValue }) => {
    try {
      return (await postData("/inspections", data)).data;
    } catch (error) {
      return rejectMessage(error, "Không thể tạo kiểm định", rejectWithValue);
    }
  },
);
export const updateInspection = createAsyncThunk(
  ...
  async ({ id, data }, { rejectWithValue }) => {
    try {
      return (await putData(`/inspections/${id}`, data)).data;
    } catch (error) {
      return rejectMessage(
        error,
        "Không thể cập nhật kiểm định",
        rejectWithValue,
      );
    }
  },
...
export const updateInspectionChecklist = action(
  "checklist",
  "put",
  "checklist",
);
...
export const deleteInspectionImage = createAsyncThunk(
  "inspections/deleteImage",
  async ({ id, imageId, data }, { rejectWithValue }) => {
    try {
      return (
        await deleteData(
          `/inspections/${id}/images/${imageId || data?.imageId}`,
        )
      ).data;
    } catch (error) {
      return rejectMessage(
        error,
        "Không thể xóa ảnh kiểm định",
        rejectWithValue,
      );
    }
  },
...
const slice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchInspections.fulfilled, (state, actionValue) => {
        state.loading = false;
        state.items = actionValue.payload || [];
      })
      .addCase(fetchInspections.rejected, (state, actionValue) => {
        state.loading = false;
        state.error = actionValue.payload;
      });
      ...
      builder
        ...
        .addCase(thunk.fulfilled, (state, actionValue) => {
          ...
          const index = state.items.findIndex(
            (item) => item.id === actionValue.payload.id,
          );
          if (index >= 0)
            state.items[index] = {
              ...state.items[index],
        ...
        .addCase(thunk.rejected, (state, actionValue) => {
          state.submitting = false;
          state.error = actionValue.payload;
        });
    }
