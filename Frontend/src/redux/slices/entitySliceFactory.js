...
export const createEntitySlice = ({ name, endpoint }) => {
  ...
  const deleteImage = createAsyncThunk(`${name}/deleteImage`, async ({ id, publicId }, { rejectWithValue }) => {
    try { return (await deleteData(`${endpoint}/${id}/images`, { public_id: publicId })).data; }
    catch (error) { return rejectWithValue(error.response?.data?.message || "Không thể xóa ảnh"); }
  });
  const slice = createSlice({
    ...
    extraReducers: (builder) => builder
      ...
      .addCase(deleteImage.fulfilled, (state, action) => { state.submitting = false; const index=state.items.findIndex((item)=>item.id===action.payload.id); if(index>=0) state.items[index]=action.payload; })
      .addMatcher((action) => [createOne.pending.type,updateOne.pending.type,deleteOne.pending.type,deleteImage.pending.type].includes(action.type), (state) => { state.submitting = true; state.error = null; })
      .addMatcher((action) => [createOne.rejected.type,updateOne.rejected.type,deleteOne.rejected.type,deleteImage.rejected.type].includes(action.type), (state, action) => { state.submitting = false; state.error = action.payload; }),
  });
  return { reducer: slice.reducer, actions: slice.actions, thunks: { fetchAll, createOne, updateOne, deleteOne, deleteImage } };
};
