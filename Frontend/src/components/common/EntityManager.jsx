...
export default function EntityManager({
  ...
  const submit = async (event) => {
    ...
    try {
      if (editing)
        await dispatch(thunks.update({ id: editing.id, data })).unwrap();
      else await dispatch(thunks.create(data)).unwrap();
      toast.success(editing ? "Cập nhật thành công" : "Tạo mới thành công");
      setModal(false);
      setImageFiles([]);
    } catch {
      /* Redux state displays the API error. */
    }
  };
  const remove = async (item) => {
    if (
      !window.confirm(
        `Xác nhận xóa ${item.name || item.full_name || item.model || "bản ghi này"}?`,
      )
    )
      return;
    try {
  ...
  const removeExistingImage = async (image) => {
    if (
      !editing ||
      !thunks.removeImage ||
      !window.confirm("Xác nhận xóa ảnh này?")
    )
      return;
    try {
      const updated = await dispatch(
        thunks.removeImage({ id: editing.id, publicId: image.public_id }),
      ).unwrap();
      setEditing(updated);
                ...
                >
                  ...
                  <td className="px-5 py-4">
                    <div className="flex justify-end gap-1">
                      <button
                        onClick={() => open(item)}
                        className="rounded-lg p-2 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-950/50"
                        title="Sửa"
                      >
                      ...
                      <button
                        onClick={() => remove(item)}
                        className="rounded-lg p-2 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/50"
                        title="Xóa"
                      >
        ...
        <form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">
          ...
          {imageUpload && (
            <div className="sm:col-span-2">
              ...
              <label className="grid cursor-pointer place-items-center rounded-2xl border-2 border-dashed border-blue-200 bg-blue-50/50 p-6 text-center dark:border-blue-900 dark:bg-blue-950/20">
                <span className="font-bold text-blue-700 dark:text-blue-400">
                  Chọn ảnh
                </span>
                <span className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                  JPEG, PNG, WEBP · tối đa {imageUpload.maxFiles} ảnh, 5 MB/ảnh
                </span>
                <input
                  type="file"
              ...
              {(editing?.images?.length > 0 || imagePreviews.length > 0) && (
                <div className="mt-3 grid grid-cols-3 gap-3 sm:grid-cols-5">
                  {(editing?.images || []).map((image) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="absolute right-1 top-1 rounded bg-rose-600 px-2 py-1 text-xs font-bold text-white"
                      >
                        Xóa
                      </button>
                    </div>
                  ...
                  {imagePreviews.map((preview) => (
                    <img
                      ...
                      src={preview.url}
                      alt={preview.name}
                      className="h-20 w-full rounded-xl object-cover ring-2 ring-blue-400"
                    />
                  ))}
                </div>
              )}
