  ...
  return <>
    <TableShell title={title} subtitle={subtitle} search={search} setSearch={(value) => { setSearch(value); setPage(1); }} onAdd={() => open()} addLabel={addLabel}>
      {loading ? <LoadingState /> : <table className="w-full min-w-[760px] text-left text-sm">
        ...
        <tbody className="divide-y divide-slate-100">{shown.map((item, index) => <tr key={item.id} className="hover:bg-slate-50/80">
          ...
          {columns.map((column) => <td key={column.key} className={`px-4 py-4 ${column.primary ? "font-bold text-slate-900" : "text-slate-600"}`}>{column.type === "status" ? <StatusBadge value={item[column.key]} /> : format(item[column.key], column.type)}</td>)}
    ...
    <Modal open={modal} onClose={() => setModal(false)} title={editing ? `Cập nhật ${title.toLowerCase()}` : `Thêm ${title.toLowerCase()}`}>
      <form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">
        ...
        {imageUpload && <div className="sm:col-span-2"><span className="mb-1.5 block text-xs font-bold text-slate-600">{imageUpload.label}</span><label className="grid cursor-pointer place-items-center rounded-2xl border-2 border-dashed border-blue-200 bg-blue-50/50 p-6 text-center"><span className="font-bold text-blue-700">Chọn ảnh</span><span className="mt-1 text-xs text-slate-500">JPEG, PNG, WEBP · tối đa {imageUpload.maxFiles} ảnh, 5 MB/ảnh</span><input type="file" accept="image/jpeg,image/png,image/webp" multiple={imageUpload.multiple} className="sr-only" onChange={selectImages} /></label>{(editing?.images?.length > 0 || imagePreviews.length > 0) && <div className="mt-3 grid grid-cols-3 gap-3 sm:grid-cols-5">{(editing?.images || []).map((image) => <img key={image.public_id || image.url} src={image.url} alt="Ảnh hiện tại" className="h-20 w-full rounded-xl object-cover" />)}{imagePreviews.map((preview) => <img key={preview.url} src={preview.url} alt={preview.name} className="h-20 w-full rounded-xl object-cover ring-2 ring-blue-400" />)}</div>}</div>}
