  ...
  return <>
    <TableShell title={title} subtitle={subtitle} search={search} setSearch={(value) => { setSearch(value); setPage(1); }} onAdd={() => open()} addLabel={addLabel}>
      {loading ? <LoadingState /> : <table className="w-full min-w-[760px] text-left text-sm">
        <thead className="bg-slate-50 text-[11px] uppercase tracking-wider text-slate-500 dark:bg-slate-900 dark:text-slate-400"><tr><th className="px-5 py-4">STT</th>{columns.map((column) => <th key={column.key} className="px-4 py-4">{column.label}</th>)}<th className="px-5 py-4 text-right">Thao tác</th></tr></thead>
        <tbody className="divide-y divide-slate-100 dark:divide-slate-700">{shown.map((item, index) => <tr key={item.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-900/60">
          <td className="px-5 py-4 text-xs text-slate-400">{(page - 1) * pageSize + index + 1}</td>
          {columns.map((column) => <td key={column.key} className={`px-4 py-4 ${column.primary ? "font-bold text-slate-900 dark:text-white" : "text-slate-600 dark:text-slate-300"}`}>{renderCell(item, column)}</td>)}
          <td className="px-5 py-4"><div className="flex justify-end gap-1"><button onClick={() => open(item)} className="rounded-lg p-2 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-950/50" title="Sửa"><Icon name="edit" className="h-4 w-4" /></button><button onClick={() => remove(item)} className="rounded-lg p-2 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/50" title="Xóa"><Icon name="trash" className="h-4 w-4" /></button></div></td>
    ...
    <Modal open={modal} onClose={() => setModal(false)} title={editing ? `Cập nhật ${title.toLowerCase()}` : `Thêm ${title.toLowerCase()}`}>
      <form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">
        {fields.map((field) => <label key={field.key} className={field.full ? "sm:col-span-2" : ""}><span className="mb-1.5 block text-xs font-bold text-slate-600 dark:text-slate-300">{field.label}{field.required && " *"}</span>{field.type === "select" ? <select required={field.required} value={form[field.key] ?? ""} onChange={(event) => setForm({ ...form, [field.key]: event.target.value })} className="form-control">{field.options.map((option) => <option key={String(option.value)} value={option.value}>{option.label}</option>)}</select> : field.type === "textarea" ? <textarea required={field.required} rows="3" value={form[field.key] ?? ""} onChange={(event) => setForm({ ...form, [field.key]: event.target.value })} className="form-control" /> : <input required={field.required} type={field.type || "text"} value={form[field.key] ?? ""} onChange={(event) => setForm({ ...form, [field.key]: event.target.value })} className="form-control" />}</label>)}
        {imageUpload && <div className="sm:col-span-2"><span className="mb-1.5 block text-xs font-bold text-slate-600 dark:text-slate-300">{imageUpload.label}</span><label className="grid cursor-pointer place-items-center rounded-2xl border-2 border-dashed border-blue-200 bg-blue-50/50 p-6 text-center dark:border-blue-900 dark:bg-blue-950/20"><span className="font-bold text-blue-700 dark:text-blue-400">Chọn ảnh</span><span className="mt-1 text-xs text-slate-500 dark:text-slate-400">JPEG, PNG, WEBP · tối đa {imageUpload.maxFiles} ảnh, 5 MB/ảnh</span><input type="file" accept="image/jpeg,image/png,image/webp" multiple={imageUpload.multiple} className="sr-only" onChange={selectImages} /></label>{(editing?.images?.length > 0 || imagePreviews.length > 0) && <div className="mt-3 grid grid-cols-3 gap-3 sm:grid-cols-5">{(editing?.images || []).map((image) => <div key={image.public_id || image.url} className="relative"><img src={image.url} alt="Ảnh hiện tại" className="h-20 w-full rounded-xl object-cover" /><button type="button" onClick={() => removeExistingImage(image)} className="absolute right-1 top-1 rounded bg-rose-600 px-2 py-1 text-xs font-bold text-white">Xóa</button></div>)}{imagePreviews.map((preview) => <img key={preview.url} src={preview.url} alt={preview.name} className="h-20 w-full rounded-xl object-cover ring-2 ring-blue-400" />)}</div>}</div>}
        <div className="mt-3 flex justify-end gap-3 sm:col-span-2"><button type="button" onClick={() => setModal(false)} className="rounded-xl border border-slate-200 px-5 py-2.5 text-sm font-semibold dark:border-slate-700 dark:text-slate-200">Hủy</button><button disabled={submitting} className="rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-bold text-white disabled:opacity-60">{submitting ? "Đang lưu..." : "Lưu dữ liệu"}</button></div>
      </form>
    </Modal>
  </>;
}
