...
export default function ProtectedRouter({ children, allowedRoles }) {
  ...
  const { isAuthenticated, user, validated } = useSelector((state) => state.auth);
  useEffect(() => {
    if (isAuthenticated && !validated) dispatch(loadCurrentUser());
  }, [dispatch, isAuthenticated, validated]);
  if (!isAuthenticated) return <Navigate to="/admin/login" replace />;
  if (!validated) return <div className="grid min-h-screen place-items-center">Đang xác thực quyền truy cập...</div>;
