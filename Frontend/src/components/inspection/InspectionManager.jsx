...
export default function InspectionManager() {
  ...
  const openCreate = async () => {
    ...
    try {
      setContracts((await getData("/contracts")).data || []);
    } catch (requestError) {
      toast.error(
        requestError.response?.data?.message || "Không thể tải danh sách xe",
      );
    } finally {
  ...
  const openDetail = async (id) => {
    ...
    try {
      setDetail((await getData(`/inspections/${id}`)).data);
    } catch (requestError) {
      toast.error(
        requestError.response?.data?.message ||
          "Không thể tải chi tiết kiểm định",
      );
    } finally {
                ...
                <tr key={inspection.id}>
                  ...
                  <td>
                    ...
                    <div className="text-xs text-slate-400">
                      {inspection.vin || `Xe #${inspection.vehicle_id}`}
                    </div>
                  </td>
                  <td>
                  ...
                  <td>
                    {inspection.inspector_name ||
                      `NV #${inspection.inspector_id}`}
                  </td>
                  <td>
                    {new Date(
                      inspection.inspected_at || inspection.created_at,
                  ...
                  <td>
                    <button
                      type="button"
                      onClick={() => openDetail(inspection.id)}
                      className="action-green inline-flex items-center gap-1.5 px-3 py-2"
                      title="Xem chi tiết"
                    >
        ...
        <form onSubmit={submitInspection} className="space-y-5">
          ...
          {form.contract_id && (
            <div className="rounded-xl bg-slate-50 p-3 text-xs text-slate-600 dark:bg-slate-900 dark:text-slate-300">
              Contract #{form.contract_id} · Order #{form.order_id} · Vehicle #
              {form.vehicle_id}
            </div>
          )}
          <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">
            ...
            <textarea
              rows="4"
              className="form-control mt-2"
              placeholder="Nhập nội dung cần lưu ý khi kiểm định..."
              value={form.notes}
              onChange={(event) =>
          ...
          <div className="rounded-xl bg-blue-50 p-3 text-xs leading-5 text-blue-700 dark:bg-blue-950/30 dark:text-blue-400">
            Người kiểm định được xác định tự động từ tài khoản Staff/Admin đang
            đăng nhập. Phiếu mới có trạng thái chờ kiểm định.
          </div>
          <button
          ...
          >
            {submitting ? "Đang tạo..." : "Tạo phiếu kiểm định"}
          </button>
        </form>
      </Modal>

      <InspectionDetailModal
        key={`${detail?.id || "none"}-${detail?.updated_at || "loading"}-${detail?.images?.length || 0}`}
        inspection={detail}
