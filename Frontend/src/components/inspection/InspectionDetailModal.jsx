...
export default function InspectionDetailModal({ inspection, loading, onClose, onRefresh }) {
  const dispatch = useDispatch();
  const submitting = useSelector((state) => state.inspections.submitting);
  const [checklist, setChecklist] = useState(() => inspection?.checklist?.length ? inspection.checklist : defaultChecklist);
  const [notes, setNotes] = useState(() => inspection?.notes || inspection?.note || "");
  ...
  const run = async (thunk, data, successMessage) => {
    try {
      ...
      toast.error(typeof error === "string" ? error : "Không thể cập nhật kiểm định");
    }
  };
  const saveChecklist = () => run(updateInspectionChecklist, { checklist, notes }, "Đã lưu checklist");
  const finish = async (result) => {
    const label = result === "pass" ? "PASS" : "FAIL";
    if (!window.confirm(`Xác nhận hoàn thành kiểm định với kết quả ${label}?`)) return;
    try {
      await dispatch(updateInspectionChecklist({ id: inspection.id, data: { checklist, notes } })).unwrap();
      ...
      toast.error(typeof error === "string" ? error : "Không thể hoàn thành kiểm định");
    }
  };
  const upload = async () => {
  ...
  const toggleChecklistItem = async (index, checked) => {
    ...
    try {
      ...
      await dispatch(updateInspectionChecklist({ id: inspection.id, data: { checklist: nextChecklist, notes } })).unwrap();
      toast.success("Đã bắt đầu kiểm định và lưu checklist");
      await onRefresh(inspection.id);
    } catch (error) {
      setChecklist(checklist);
      toast.error(typeof error === "string" ? error : "Không thể bắt đầu kiểm định");
      await onRefresh(inspection.id);
    }
  };

  return <Modal open={Boolean(inspection) || loading} onClose={onClose} title="Chi tiết kiểm định" size="max-w-5xl">
    {loading || !inspection ? <LoadingState /> : <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-2xl bg-slate-900 p-5 text-white sm:flex-row sm:items-center sm:justify-between">
        <div><div className="text-xs font-bold uppercase tracking-wider text-slate-400">Mã kiểm định</div><div className="mt-1 text-2xl font-black">KD{String(inspection.id).padStart(5, "0")}</div><div className="mt-2 text-sm text-slate-300">{[inspection.brand, inspection.model, inspection.vin].filter(Boolean).join(" · ")}</div></div>
        <StatusBadge value={inspection.status} />
      </div>

      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Info label="Người kiểm định" value={inspection.inspector_name || `NV #${inspection.inspector_id}`} />
        <Info label="Ngày tạo phiếu" value={dateTime(inspection.created_at)} />
        <Info label="Ngày hoàn thành" value={dateTime(inspection.inspected_at || inspection.inspection_date)} />
        <Info label="Đơn hàng liên quan" value={inspection.order_number ? `DH${String(inspection.order_number).padStart(5, "0")}` : "Chưa liên kết"} />
