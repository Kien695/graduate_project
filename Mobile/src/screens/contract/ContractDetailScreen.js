...
export default function ContractDetailScreen({ navigation, route }) {
  ...
  const load = useCallback(async () => {
    try {
      setError("");
      setContract(await getContractById(route.params.contractId));
    } catch (requestError) {
      setError(
  ...
  const handleConfirm = async () => {
    setConfirming(true);
    try {
      setContract(await confirmContract(contract.id));
      Alert.alert("Thành công", "Bạn đã xác nhận hợp đồng.");
    } catch (requestError) {
      Alert.alert(
        "Không thể xác nhận",
    ...
    return (
      <SafeAreaView style={styles.safe}>
        ...
        <View style={styles.center}>
          <Text style={styles.loading}>{error || "Đang tải..."}</Text>
        </View>
      </SafeAreaView>
    );
  const canConfirm = String(contract.status).toUpperCase() === "APPROVED";
        ...
        >
          ...
          <View style={styles.heading}>
            <View style={styles.headingText}>
              <Text style={styles.code}>{contract.contract_number}</Text>
          ...
          <View style={styles.card}>
            ...
            <Row label="Mã hợp đồng" value={contract.contract_number} />
            <Row
              label="Xe"
              value={`${contract.brand || ""} ${contract.model || ""}`.trim()}
            />
            <Row label="Khách hàng" value={contract.customer_name} />
...
const getStyles = (colors) =>
  StyleSheet.create({
    ...
    heading: { flexDirection: "row", alignItems: "flex-start", marginTop: 20 },
    headingText: { flex: 1, paddingRight: 12 },
    code: { color: colors.text, fontSize: 20, fontWeight: "900" },
    vehicle: {
      color: colors.muted,
      fontSize: 14,
      marginTop: 5,
    ...
    terms: { color: colors.muted, fontSize: 13.5, lineHeight: 21 },
    footer: {
      position: "absolute",
      left: 0,
      right: 0,
